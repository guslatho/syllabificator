# See Trogkanis 2010 for detailed description of various measures used

import pandas as pd
import numpy as np

test_df1 = pd.Series([1,2,3,4,5])
test_df2 = pd.Series([1,2,3,4,5])

# dataset_correct = pd.read_csv('trog.csv')
# dataset_handmatig = pd.read_csv('py_default.csv')

# Analyze correct word set against input set(s)
# Takes '-' as valid hyphenation character on intput
...
# analyze_set(cset, tset1, tset2)

cset = pd.Series(['ba-zoo-wie','kam-pi-oen','tra-la-la','vloo-ien','trein-tje'])
tset1 = pd.Series(['bazoo-wie','kam-pi-oen','tra-lala','vlooien','trein-tje'])
tset2 = pd.Series(['ba-zoo-w-ie','ka-mpi-oen','tr-al-ala','vl-ooien','trei-ntje'])

cset = pd.Series(['scha-ken','drin-ken','school-ter-rein','vlooi-en','pam-flet'])
tset1 = pd.Series(['schaken','drink-en','school-ter-rein','vlooi-en','pam-flet'])
tset2 = pd.Series(['sc-ha-ken','drink-en','school-ter-rein','vlooi-en','pam-flet'])
tset3 = pd.Series(['scha-ken','drin-ken','school-ter-rein','vlooi-en','pam-flet'])
