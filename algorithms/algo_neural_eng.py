...
E = keras.layers.TimeDistributed(keras.layers.Embedding(input_dim=ALPHABET_SIZE, output_dim=128))(inputs)
C = keras.layers.TimeDistributed(keras.layers.Conv1D(filters=100, kernel_size=4, strides=1, padding='same'))(E)
C = keras.layers.TimeDistributed(keras.layers.MaxPooling1D(pool_size=4, strides=1, padding='same'))(C)
...
model_path = os.path.join(os.path.dirname(__file__), '..', 'models', 'eng_neural_model.h5')
