...
# Use the current directory to load dictionaries
# MODIFIED TO LOAD DICTIONARIES IN CURRENT DIRECTORY
dictionaries = Path(__file__).parent
...
# Function to hyphenate a word using a specified dictionary file
def liang_syllabificate(word):
    # Define the path to the dictionary file:
    # Establish model path, load
    model_path = os.path.join(os.path.dirname(__file__), '..', 'models', 'liang_custom.dic')
    
    dictionary_path = Path(__file__).parent / model_path

    # Create a Pyphen instance with the specific dictionary file
    hyphenator = Pyphen(filename=dictionary_path, left=1, right=2)
